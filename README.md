...
[📐 Arquitectura](#-arquitectura) · [🚀 Quick Start](#-quick-start) · [🤖 Modelos IA](#-modelos-de-ia) · [📊 Dashboards](#-dashboards-power-bi) · [🗃️ Modelo de Datos](#️-modelo-de-datos) · [📁 Estructura](#-estructura-del-proyecto)
...
│                                                                                 │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌────────┐ ┌──────┐ ┌─────────────┐  │
│  │ Facebook │ │Instagram │ │ WhatsApp │ │ Email  │ │ Chat │ │  Tickets    │  │
│  │  JSON    │ │  JSON    │ │  JSON    │ │  CSV   │ │  CSV │ │  Internos   │  │
│  └────┬─────┘ └────┬─────┘ └────┬─────┘ └───┬────┘ └──┬───┘ └──────┬──────┘  │
│       └────────────┴────────────┴────────────┴─────────┴────────────┘          │
│                                        │                                        │
...
│  │  • Business rules validation       • Separación external/internal        │  │
│  │                                                                          │  │
│  └──────────────────────────────────────────────────────────────────────────┘  │
│                                        │                                        │
...
│                                                                                 │
│         ┌──────────┐          ┌─────────────────┐          ┌──────────────┐   │
│         │ STAGING  │ ───────► │    DWH CORE     │ ───────► │ DATA MARTS   │   │
│         │          │          │                 │          │              │   │
│         │ raw data │          │ fact_tickets    │          │ cx_mart      │   │
│         │ sin       │         │ fact_interact.  │          │ hr_mart      │   │
│         │ procesar │          │ fact_daily_ch.  │          │ exec_mart    │   │
│         └──────────┘          │                 │          └──────────────┘   │
│                               │ + 9 dimensiones │                             │
│                               └─────────────────┘                             │
│                                        │                                       │
│  ④ CAPA DE IA (Python — 4 módulos)                                            │
│                                                                                │
│  ┌───────────────────┐  ┌──────────────────┐  ┌─────────────────┐  ┌───────┐ │
│  │  Ticket           │  │  Sentiment       │  │  SLA Breach     │  │  LLM  │ │
│  │  Classifier       │  │  Analyzer        │  │  Predictor      │  │  Exec │ │
│  │                   │  │                  │  │                 │  │  Sum. │ │
│  │  TF-IDF +         │  │  TF-IDF + LogReg │  │  XGBoost        │  │       │ │
│  │  Logistic Reg.    │  │  (sustituto de   │  │  AUC 0.65       │  │  API  │ │
│  │  F1 0.75          │  │  BERT) F1 0.99   │  │  (post-fix)     │  │       │ │
│  └───────────────────┘  └──────────────────┘  └─────────────────┘  └───────┘ │
│                                        │                                       │
│  ⑤ VISUALIZACIÓN (Power BI Desktop)                                           │
│                                                                                │
│  ┌──────────────────────────────┐   ┌──────────────────────────────────────┐  │
│  │   CX Operations Board        │   │   Employee Service Dashboard         │  │
│  │   (Gerentes / Supervisores)  │   │   (RRHH / TI / Operaciones)          │  │
│  │                              │   │                                      │  │
│  │   • KPIs en tiempo real      │   │   • Backlog aging por área           │  │
│  │   • Alert: tickets en riesgo │   │   • SLA interno por categoría        │  │
│  │   • Heatmap volumen × hora   │   │   • Reincidencia por departamento    │  │
│  │   • Sentiment trend          │   │   • Tickets duplicados detectados    │  │
│  └──────────────────────────────┘   └──────────────────────────────────────┘  │
│                                                                                │
│  ⑥ MONITORING                                                                  │
│  ┌──────────────────────────────────────────────────────────────────────────┐  │
│  │  Data Quality Monitor  ·  Pipeline Health  ·  Model Drift Detector       │  │
│  └──────────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────────┘
```
